...
# Step 7: Create a line extending horizontally from the point's longitude to the intersection points
# Replicate longitude to match the number of intersection points
longitude_replicated <- rep(longitude, nrow(intersection_coords))

# Create line coordinates with replicated longitude
line_coords <- cbind(longitude_replicated, intersection_coords[, "Y"])
